...
public class BytesUtil {
	...
	//Corriente de bytes a cadena hexadecimal
	...
	//Solo carácter a byte
	private static byte charToByte(char c) {
		return (byte) "0123456789ABCDEF".indexOf(c);
	}

	//Cadena hexadecimal a matriz de bytes
	...
	public static byte[] getBytesFromHexString(String hexstring){
		...
		for(int i=0; i<size; i++){
			int pos = i * 2;
			rv[i] = (byte) (charToByte(hexarray[pos]) << 4 | (charToByte(hexarray[pos + 1]) & 0xff));
		}
		return rv;
	}

	//Cadena decimal a matriz de bytes
	@SuppressLint("DefaultLocale")
	...
	//Operación de combinación de matrices de bytes 1
	...
	//Operación de combinación de matrices de bytes 2
	...
	//Generar flujo de bytes de tabla
	...
	/**
	 * Generar un flujo de bytes de código bidimensional
	 *
	 * @param data
	 * @param size
	 * @return
	...
	/**
	 * Convierta el mapa de bits a una imagen raster con los primeros cuatro dígitos de ancho y alto
	 */
	public static byte[] getBytesFromBitMap(Bitmap bitmap) {
		...
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				...
				rv[(width * i + j) / 8 + 4] = (byte) (rv[(width * i + j) / 8 + 4]& 0xff | (gray << (7 - ((width * i + j) % 8))));
			}
		}
	...
	/**
	 * Convertir mapa de bits a datos de línea de punto N especificados por el modo
	 */
			...
			for(int i = 0; i < height/8; i++){
				...
				for(int j = 0; j < width; j++){
					...
					for(int m = 0; m < 8; m++){
						...
						int green = (clr & 0x0000ff00) >> 8;
						int blue = clr & 0x000000ff;
						gray = (byte) ((RGB2Gray(red, green, blue)<<(7-m))|gray & 0xff);
					}
	...
	/**
	 * Generar datos de bloques negros intermitentes
	 * * @param w: ancho de papel, punto de unidad
	 * @return
	 */
	...
	/**
	 * Generar un bloque grande de datos de bloque negro
	 * * @param h: altura del bloque negro, punto de unidad
	 * * @param w: ancho de bloque negro, punto de unidad, múltiplo de 8
	 * @return
	 */
	...
	//Billete de grupo de Estados Unidos
	...
	//Cobertura básica de todas las instrucciones de epson en la impresión térmica de tickets.
	public static byte[] customData(){
		byte[] rv = new byte[]{
				...
				0x2D, 0x2D,0x2D,0x2D,0x2D,0x2D,0x2D, 0x2D,0x2D,0x2D,0x2D,0x2D,0x2D, 0x2D,0x2D,0x2D,0x2D,0x2D,0x2D, 0x2D,0x2D,0x2D,0x2D,0x2D,0x2D,0x2D,0x2D, 0x2D,0x2D,0x2D,0x2D,0x2D,0x0A,
				//Establecer el espaciado correcto del carácter
				...
				0x3D, 0x3D, 0x3D, 0x3D,0x3D, 0x3D, 0x3D, 0x3D,0x3D, 0x3D, 0x3D, 0x3D,0x3D, 0x3D, 0x3D, 0x3D,0x3D, 0x3D, 0x3D, 0x3D,0x3D, 0x3D, 0x3D, 0x3D,0x3D, 0x3D, 0x3D, 0x3D,0x3D, 0x3D, 0x3D, 0x3D, 0x0A,
				//Efecto de fuente
				//// Establecer negrita, doble altura, doble ancho, subrayado, blanco inverso
				...
				0x2A, 0x2A, 0x2A, 0x2A,0x2A, 0x2A, 0x2A, 0x2A,0x2A, 0x2A, 0x2A, 0x2A,0x2A, 0x2A, 0x2A, 0x2A,0x2A, 0x2A, 0x2A, 0x2A,0x2A, 0x2A, 0x2A, 0x2A,0x2A, 0x2A, 0x2A, 0x2A,0x2A, 0x2A, 0x2A, 0x2A, 0x0A,
				//Posición de composición
				/// / Establecer la posición absoluta y establecer la posición relativa
				...
				//Establecer el ancho del área de impresión
				...
				0x1B, 0x61, 0x00,
				//Línea de división ---	  (Un método para imprimir el mapa de bits ráster se implementará más adelante, nuevamente antes de usar 1b 61 01 centrado）
				0x2D, 0x2D,0x2D,0x2D,0x2D,0x2D,0x2D, 0x2D,0x2D,0x2D,0x2D,0x2D,0x2D, 0x2D,0x2D,0x2D,0x2D,0x2D,0x2D, 0x2D,0x2D,0x2D,0x2D,0x2D,0x2D,0x2D,0x2D, 0x2D,0x2D,0x2D,0x2D,0x2D, 0x0A,
	...
	//Carácter parcial impreso marca de línea de división
