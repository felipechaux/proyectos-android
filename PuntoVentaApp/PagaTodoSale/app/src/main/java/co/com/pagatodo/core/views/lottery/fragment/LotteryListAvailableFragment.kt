...
class LotteryListAvailableFragment : Fragment() {

    interface OnFragmentInteractionListener {
        fun showMessageDialog(title: String, message: String, backMenu: Boolean)
    }

    private var listener: LotteryListAvailableFragment.OnFragmentInteractionListener? = null
    ...
    companion object {
        fun newInstance(activity: LotteryAdapter.OnListenerAdapter, fragmentListener: OnFragmentInteractionListener) =
            LotteryListAvailableFragment().apply {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        return inflater.inflate(R.layout.fragment_lottery_list_available, container, false)
    }
            ...
            when(it) {
                is LotterySaleViewModel.ViewEvent.ResponseLotteries -> {
                    if(it.virtuaLotteries.isEmpty()){
                        listener?.showMessageDialog(
                            R_string(R.string.title_dialog_empty_lotteries),
                            R_string(R.string.message_dialog_empty_lotteries), true)
                    }else{
